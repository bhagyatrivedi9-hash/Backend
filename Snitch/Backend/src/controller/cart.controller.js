...
export const incrementQuantity= async (req,res)=>{
        ...
        const itemQuantity= cart.items.find(item=> item.product.toString() === productId && item.variant.toString() === variantId)?.quantity || 0

        if(itemQuantity + 1 > stock){
            return res.status(400).json({ message: `Only ${stock} items left in stock. and you already have ${itemQuantity} items in your cart`, success: false })
        }
...
export const decrementQuantity= async (req,res)=>{
   const { productId, variantId } = req.params
        ...
        const itemQuantity= cart.items.find(item=> item.product.toString() === productId && item.variant.toString() === variantId)?.quantity || 0
        if(itemQuantity - 1 < 1){
            return res.status(400).json({ message: `Quantity cannot be less than 1`, success: false })
        }
